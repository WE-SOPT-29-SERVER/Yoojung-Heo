...
const { Pool, Query } = require('pg'); // 포스트그레스
const dayjs = require('dayjs'); // 날짜를 다루기 편하게 해주는 모듈
...
// .env에 있는 것은 process.env로 접근 가능
// NODE_ENV는 nodejs환경에서 기본적으로 설정해준 환경변수
...
const connect = async (req) => { // connect 함수 재정의
  const now = dayjs();
  const string =
    !!req && !!req.method
  ...
  client.release = () => { // release가 너무 늦게 일어났을 때
